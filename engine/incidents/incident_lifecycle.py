...
from engine.storage.media_recorder import media_recorder
from engine.storage.storage_queue import storage_queue
...
class IncidentLifecycleManager:
    ...
    def set_socket_emitter(self, emitter):
        ...
        situation_engine.set_socket_emitter(emitter)

    def set_flask_app(self, app):
        self.flask_app = app
        situation_engine.set_app(app)
        ...
        with self.lock:
            ...
            for t in tracks:
                ...
                if not session:
                    ...
                    # 7. Correlate into Stage 2 Security Situation Engine
                    try:
                        situation_engine.process_incident(inc_id, initial_pkg)
                    except Exception as ex:
                        app_logger.error(f"[SITUATION_CORRELATION_ERROR] #{inc_id}: {ex}")
    ...
    def process_tamper(
        ...
        with self.lock:
            ...
            if is_tamper_active and not session:
                ...
                # Capture & verify tamper snapshot (use clear pre-tamper frame if available to provide useful identification context)
                snap_frame = current_frame
                if pre_event_buffer and len(pre_event_buffer) > 0:
                    # Use last clear frame before obstruction
                    snap_frame = pre_event_buffer[-1]

                snap_res = media_recorder.capture_snapshot(snap_frame, inc_id, prefix='tamper')
                snap_filename = snap_res.file_name if snap_res.valid else ''
                snap_status = 'AVAILABLE' if snap_res.valid else 'FAILED'

                # Immediately encode and persist pre-tamper 10-second video evidence (T-10s -> T-0)
                clip_filename = ''
                clip_status = 'FAILED'
                if pre_event_buffer and len(pre_event_buffer) > 0:
                    clip_res = media_recorder.encode_pre_event_clip(
                        frames=pre_event_buffer,
                        incident_id=inc_id,
                        camera_id=camera_id,
                        fps=20.0,
                        prefix='tamper_pre_event'
                    )
                    if clip_res.valid:
                        clip_filename = clip_res.file_name
                        clip_status = 'AVAILABLE'
                    else:
                        clip_status = 'FAILED'
                else:
                    app_logger.warning(f"[TAMPER_PRE_EVENT_EMPTY] No pre-event frames available for Camera {camera_id} Incident #{inc_id}")

                evidence_status = 'COMPLETE' if (snap_res.valid and clip_status == 'AVAILABLE') else 'PARTIAL' if (snap_res.valid or clip_status == 'AVAILABLE') else 'FAILED'

                # Start tamper video session preserving pre-tamper frames for live session tracking
                media_recorder.start_video_session(session_key, inc_id, pre_event_buffer)
                ...
                session.video_status = clip_status
                session.evidence_status = evidence_status
                session.timeline = initial_pkg['timeline']
                self.active_sessions[session_key] = session

                # Update DB with immediately available pre-event clip and snapshot
                db_manager.update_incident_lifecycle(
                    self.flask_app,
                    inc_id,
                ...
                if self.socket_emitter:
                    if alert_dict: self.socket_emitter('alert_created', alert_dict)
                    if evidence_dict: self.socket_emitter('evidence_created', evidence_dict)
                    self.socket_emitter('tamper_started', {
                        'camera_id': camera_id,
                        'incident_id': inc_id,
                        'reasons': reasons,
                        'has_clip': bool(clip_status == 'AVAILABLE'),
                        'clip_url': f'/api/evidence/{inc_id}/media/clip' if clip_status == 'AVAILABLE' else None
                    })

                # Correlate into Stage 2 Security Situation Engine
                try:
                    situation_engine.process_incident(inc_id, initial_pkg)
                except Exception as ex:
                    app_logger.error(f"[SITUATION_CORRELATION_ERROR] Tamper #{inc_id}: {ex}")

                app_logger.warning(f"[TAMPER_STARTED] Camera {camera_id} Tamper #{inc_id}: {', '.join(reasons)} | Pre-Event Video: {clip_filename or 'UNAVAILABLE'}")
                app_logger.info(f"[TAMPER_INCIDENT_CREATED] incident_id={inc_id} clip={clip_filename} status={evidence_status}")
    ...
    def _finalize_and_close_session(self, session_key: str, session: IncidentSession, exit_reason: str):
        ...
        """
        # If pre-event clip was already generated and validated upon trigger (e.g. tamper incident),
        # preserve the pre-event evidence rather than overwriting with black/obstructed frames
        if session.clip_path and session.video_status == 'AVAILABLE':
            clip_filename = session.clip_path
            video_status = 'AVAILABLE'
            evidence_status = session.evidence_status or 'COMPLETE'
            # Clean up media_recorder active session buffer
            with media_recorder.lock:
                media_recorder.active_sessions.pop(session_key, None)
        else:
        ...
        # Check active situations after incident closure
        try:
            situation_engine.evaluate_active_situations(quiet_period_seconds=15.0)
        except Exception:
            pass

    def force_resolve_session(self, incident_id: int) -> bool:
        """
        Manually triggered by operator resolving an alert from UI.
        If an active recording session exists for this incident, finalizes the MP4 video immediately.
        """
        with self.lock:
            found_key = None
            found_session = None
            for key, sess in self.active_sessions.items():
                if sess.incident_id == incident_id:
                    found_key = key
                    found_session = sess
                    break

            if found_key and found_session:
                self._finalize_and_close_session(found_key, found_session, exit_reason="Resolved manually by Operator")
                return True
            return False
