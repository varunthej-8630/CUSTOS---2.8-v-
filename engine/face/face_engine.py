...
from engine.face.face_quality import face_quality_validator
...
class FaceRecognitionEngine:
    ...
    def recognize_person_in_bbox(
        ...
        # Proportional contextual framing for identification cards & appearance history (head, hair, upper body)
        pad_x = int(fw * 0.45)
        pad_top = int(fh * 0.40)
        pad_bottom = int(fh * 0.70)
        cx1 = max(0, full_fx - pad_x)
        cy1 = max(0, full_fy - pad_top)
        cx2 = min(img_w, full_fx + fw + pad_x)
        cy2 = min(img_h, full_fy + fh + pad_bottom)
        face_crop_img = frame[cy1:cy2, cx1:cx2].copy()
    ...
    def validate_uploaded_image(self, img_bgr: np.ndarray) -> Tuple[bool, Optional[np.ndarray], Optional[np.ndarray], float, str]:
        ...
        # Extract embedding via SFace using raw detected landmarks
        ...
        # Generate proportional contextual portrait crop for UI display (head, hair, upper body)
        bx, by, bw, bh = primary_face['box']
        pad_x = int(bw * 0.45)
        pad_top = int(bh * 0.40)
        pad_bottom = int(bh * 0.70)
        cx1 = max(0, bx - pad_x)
        cy1 = max(0, by - pad_top)
        cx2 = min(w, bx + bw + pad_x)
        cy2 = min(h, by + bh + pad_bottom)
        display_face = img_bgr[cy1:cy2, cx1:cx2].copy()

        return True, embedding, display_face, quality_score, "Face verified successfully"
