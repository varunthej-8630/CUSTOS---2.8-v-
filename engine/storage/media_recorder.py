...
class IncidentMediaRecorder:
    ...
    def finalize_video(self, session_key: str, fps: int = 20, prefix: str = 'incident') -> MediaVerificationResult:
        ...
        try:
            writer = None
            # Prioritize browser-compatible H.264 encoding for HTML5 <video> playback
            # 1. On Windows, use Microsoft Media Foundation (MSMF) H264 hardware/software encoder
            if hasattr(cv2, 'CAP_MSMF'):
                for codec in ['H264', 'avc1']:
                    try:
                        fourcc = cv2.VideoWriter_fourcc(*codec)
                        w_test = cv2.VideoWriter(full_path, cv2.CAP_MSMF, fourcc, float(fps), (w, h))
                        if w_test.isOpened():
                            writer = w_test
                            break
                    except Exception:
            ...
            # 2. Try standard backend H264 / avc1
            ...
            # 3. Fallback to mp4v if H.264 is unavailable
            if writer is None or not writer.isOpened():
    ...
    def encode_pre_event_clip(
        self,
        frames: List[np.ndarray],
        incident_id: Any,
        camera_id: int = 0,
        fps: float = 20.0,
        prefix: str = 'tamper_pre_event'
    ) -> MediaVerificationResult:
        """
        Encodes pre-event rolling buffer frames into a browser-compatible H.264 MP4 video
        immediately upon tamper detection.
        Ensures collision-safe filename, verified readability on disk, and non-zero duration.
        """
        if not frames:
            app_logger.error(f"[TAMPER_VIDEO_FAILED] No pre-event frames provided for Incident #{incident_id}")
            return MediaVerificationResult(False, '', '', 0, error='No pre-event frames available')

        # Clean non-empty frames
        valid_frames = [f for f in frames if f is not None and f.size > 0]
        if not valid_frames:
            app_logger.error(f"[TAMPER_VIDEO_FAILED] All pre-event frames were empty for Incident #{incident_id}")
            return MediaVerificationResult(False, '', '', 0, error='All pre-event frames were empty')

        ts = time.strftime('%Y%m%d_%H%M%S')
        file_name = f"{prefix}_cam{camera_id}_inc{incident_id}_{ts}.mp4"
        full_path = os.path.join(self.snapshot_dir, file_name)

        h, w = valid_frames[0].shape[:2]
        fps_val = max(5.0, min(60.0, float(fps)))

        app_logger.info(
            f"[TAMPER_CONFIRMED] camera_id={camera_id} incident_id={incident_id} "
            f"detection_time={time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        app_logger.info(
            f"[PRE_EVENT_FRAMES_SELECTED] frames_selected={len(valid_frames)} fps={fps_val} "
            f"estimated_duration={round(len(valid_frames)/fps_val, 2)}s"
        )
        app_logger.info(f"[ENCODING_STARTED] target={file_name}")
        ...
        try:
            ...
            if hasattr(cv2, 'CAP_MSMF'):
                for codec in ['H264', 'avc1']:
                    try:
                        fourcc = cv2.VideoWriter_fourcc(*codec)
                        w_test = cv2.VideoWriter(full_path, cv2.CAP_MSMF, fourcc, fps_val, (w, h))
                        if w_test.isOpened():
                            writer = w_test
                            break
                    except Exception:
            ...
            if writer is None or not writer.isOpened():
                for codec in ['avc1', 'H264', 'x264']:
                    try:
                        fourcc = cv2.VideoWriter_fourcc(*codec)
                        w_test = cv2.VideoWriter(full_path, fourcc, fps_val, (w, h))
                        if w_test.isOpened():
                            writer = w_test
                            break
                    except Exception:
            ...
            if writer is None or not writer.isOpened():
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                writer = cv2.VideoWriter(full_path, fourcc, fps_val, (w, h))

            if not writer.isOpened():
                app_logger.error(f"[TAMPER_VIDEO_FAILED] Could not initialize OpenCV VideoWriter for {full_path}")
                return MediaVerificationResult(False, full_path, file_name, 0, error='VideoWriter open failed')
            ...
            # Post-write physical validation
            val_res = self.validate_video(full_path)
            if not val_res.valid:
                app_logger.error(f"[TAMPER_VIDEO_FAILED] Video validation failed: {val_res.error}")
                return val_res

            app_logger.info(
                f"[ENCODING_COMPLETED] file_path={file_name} file_size={val_res.file_size} bytes "
                f"frames={val_res.frame_count} fps={val_res.fps} duration={val_res.duration_sec}s"
            )
            app_logger.info(f"[MEDIA_VALIDATION] PASS clip_available=TRUE")
            return val_res

        except Exception as e:
            app_logger.error(f"[TAMPER_VIDEO_FAILED] Exception encoding pre-event video: {e}")
            return MediaVerificationResult(False, full_path, file_name, 0, error=str(e))
