...
class CircularFrameBuffer:
    ...
    def get_last_seconds(self, seconds: float = 10.0) -> List[np.ndarray]:
        """
    ...
    def get_pre_event_window(self, seconds: float = 10.0) -> Tuple[List[np.ndarray], float, float, float]:
        """
        Retrieves frames and timing metadata (frames, start_ts, end_ts, estimated_fps)
        for the requested pre-event window before tamper/incident detection.
        """
        now = time.time()
        cutoff = now - float(seconds)

        with self.lock:
            selected = [bf for bf in self.buffer if bf.timestamp >= cutoff]
            if not selected and len(self.buffer) > 0:
                est_count = min(len(self.buffer), int(seconds * 25))
                selected = list(self.buffer)[-est_count:]

            if not selected:
                return [], now, now, 20.0

            frames = [bf.frame for bf in selected]
            start_ts = selected[0].timestamp
            end_ts = selected[-1].timestamp
            elapsed = max(0.01, end_ts - start_ts)
            fps = len(frames) / elapsed if elapsed >= 0.5 and len(frames) >= 5 else 20.0
            fps = max(5.0, min(60.0, fps))
            return frames, start_ts, end_ts, round(fps, 1)
